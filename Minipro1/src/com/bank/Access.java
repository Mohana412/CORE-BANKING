...
public class Access {
		...
		public boolean loginAccount(Transfer transfer)throws Exception
		{
			...
			sql="select * from banking1 where user_name='"+u_name+"' and pin="+pin;
	...
	public boolean createAccount(Transfer transfer) throws Exception
	{
		...
		try
		{
			...
			ac+=1;
			sql="insert into banking1(acc_no,user_name,balance,pin) values("+ac+",'"+u_name+"' ,500, "+pin+")";
	...
	public void getBalance(int accountNumber) throws Exception 
	{
		try
		{
		 	sql= "select * from banking1 where acc_no="+accountNumber;
			PreparedStatement st= connect.prepareStatement(sql);
			ResultSet rs= st.executeQuery(sql);
	...
	public void withdrawAmount(int accountNumber) throws Exception
	{
	    try
	    {
	       ...
	       sql="update banking1 set balance='"+finalAmount+"' where acc_no="+accountNumber;
